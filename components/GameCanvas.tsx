...
export const GameCanvas: React.FC<GameCanvasProps> = ({ gameState, onGameOver, setScore, setCollectiblesCount, recoverSignal }) => {
  ...
  const updatePhysics = (frameCount: number) => {
    ...
    if (player.y > CANVAS_HEIGHT) {
      playDeathSound();
      onGameOver(Math.floor(scoreRef.current), collectiblesCountRef.current, 'Fell into the abyss');
      return;
    }
  ...
  const draw = (ctx: CanvasRenderingContext2D, frameCount: number) => {
    ...
    if (dayFactor > 0.1) {
      ...
      cloudsRef.current.forEach(cloud => {
        ...
        ctx.globalAlpha = Math.max(0.4, dayFactor); // More solid

        const r1 = cloud.height / 2;
        const r2 = cloud.height * 0.4;
        const r3 = cloud.height * 0.3;

        ctx.beginPath();
        // Main central circle
        ctx.arc(drawX, cloud.y, r1, 0, Math.PI * 2);
        // Left circles
        ctx.arc(drawX - r1 * 0.7, cloud.y + r1 - r2, r2, 0, Math.PI * 2);
        ctx.arc(drawX - r1 * 1.2, cloud.y + r1 - r3, r3, 0, Math.PI * 2);
        // Right circles
        ctx.arc(drawX + r1 * 0.7, cloud.y + r1 - r2, r2, 0, Math.PI * 2);
        ctx.arc(drawX + r1 * 1.2, cloud.y + r1 - r3, r3, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillRect(drawX - r1 * 1.2, cloud.y + r1 - Math.max(r2, r3), r1 * 2.4, Math.max(r2, r3));
      });
      ctx.restore();
    }
    ...
    // --- Air Traffic (Balloons & Airplanes) ---
    ctx.save();
    ctx.globalAlpha = 1 - dayFactor * 0.3; // Slightly fade at night

    // Airplane
    const airplaneCycle = 15000;
    const airplaneWorldX = frameCount * 4;
    const airplaneScreenX = (airplaneWorldX - cameraXRef.current * 0.1) % airplaneCycle;
    const drawAirplaneX = airplaneScreenX < -500 ? airplaneScreenX + airplaneCycle : airplaneScreenX;

    if (drawAirplaneX > -200 && drawAirplaneX < CANVAS_WIDTH + 200) {
      ctx.save();
      ctx.translate(drawAirplaneX, CANVAS_HEIGHT * 0.15 + Math.sin(frameCount * 0.05) * 5);
      ...
      ctx.lineTo(-30, -8);
      ctx.quadraticCurveTo(-40, -8, -40, -4);
      ctx.lineTo(-40, 4);
      ctx.quadraticCurveTo(-40, 8, -30, 8);
      ctx.lineTo(30, 8);
      ctx.quadraticCurveTo(45, 5, 40, 0);
      ctx.fill();

      // Tail
      ctx.fillStyle = '#ef4444'; // Red tail
      ctx.beginPath();
      ctx.moveTo(-30, -8);
      ctx.lineTo(-40, -25);
      ctx.lineTo(-25, -25);
    ...
    // Hot Air Balloons
    const balloonColors = ['#ef4444', '#3b82f6', '#10b981', '#a855f7']; // Red, Blue, Green, Purple
    for (let i = 0; i < 4; i++) {
      const spacing = 10000;
      const totalWidth = spacing * 4;
      const balloonParallax = cameraXRef.current * 0.15;
      const xOffset = (i * spacing + 2000) - balloonParallax;
      const screenX = xOffset % totalWidth;
      const drawX = screenX < -200 ? screenX + totalWidth : screenX;

      if (drawX > -100 && drawX < CANVAS_WIDTH + 100) {
        const balloonY = CANVAS_HEIGHT * 0.25 + Math.sin(frameCount * 0.02 + i) * 20;
        ctx.save();
        ctx.translate(drawX, balloonY);
        ctx.fillStyle = balloonColors[i];
        ctx.beginPath();
        ctx.arc(0, 0, 30, Math.PI, 0);
        ctx.bezierCurveTo(30, 40, 15, 50, 10, 60);
        ctx.lineTo(-10, 60);
        ctx.bezierCurveTo(-15, 50, -30, 40, -30, 0);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(-10, 60);
        ctx.bezierCurveTo(-10, 40, -5, 0, 0, -30);
        ctx.bezierCurveTo(5, 0, 10, 40, 10, 60);
        ctx.fill();

        // Basket
        ctx.fillStyle = '#854d0e';
        ctx.fillRect(-8, 65, 16, 12);
        ctx.strokeStyle = '#000';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(-8, 60); ctx.lineTo(-8, 65);
        ctx.moveTo(8, 60); ctx.lineTo(8, 65);
        ctx.stroke();
        ctx.restore();
      }
    ...
    for (let i = startBuildingIdx; i < endBuildingIdx; i++) {
      ...
      if (offset - mtnParallax < CANVAS_WIDTH && offset + 200 - mtnParallax > -200) {
        ...
        ctx.save();
        ctx.translate(offset, CANVAS_HEIGHT);
        ...
        if (buildingType === 0) {
          ...
          const startY = peakY - poleH;
          const rightX = peakX + flagW;

          // Black stripe
          ctx.fillStyle = '#000000';
          ctx.beginPath();
          ctx.moveTo(peakX, startY);
          ctx.lineTo(rightX, startY + wave);
          ctx.lineTo(rightX, startY + flagH/3 + wave);
          ctx.lineTo(peakX, startY + flagH/3);
          ctx.fill();

          // Red stripe
          ctx.fillStyle = '#dc2626';
          ctx.beginPath();
          ctx.moveTo(peakX, startY + flagH/3);
          ctx.lineTo(rightX, startY + flagH/3 + wave);
          ctx.lineTo(rightX, startY + (flagH/3)*2 + wave);
          ctx.lineTo(peakX, startY + (flagH/3)*2);
          ctx.fill();

          // Green stripe
          ctx.fillStyle = '#16a34a';
          ctx.beginPath();
          ctx.moveTo(peakX, startY + (flagH/3)*2);
          ctx.lineTo(rightX, startY + (flagH/3)*2 + wave);
          ctx.lineTo(rightX, startY + flagH + wave);
          ctx.lineTo(peakX, startY + flagH);
          ctx.fill();

          // White Separators
          ctx.fillStyle = '#ffffff';
          ctx.fillRect(peakX, startY + flagH/3 - 0.5, flagW, 1);
          ctx.fillRect(peakX, startY + (flagH/3)*2 - 0.5, flagW, 1);
